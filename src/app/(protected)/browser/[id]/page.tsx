...
export default function BrowserView({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <BackgroundTexture />
      <div className="relative z-10 min-h-screen bg-gray-50/50 backdrop-blur-sm p-3 sm:p-4 md:p-6">
        {/* Header */}
        <div className="max-w-7xl mx-auto mb-4 md:mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <button 
              onClick={() => router.push('/dashboard')}
              className="flex items-center gap-1.5 bg-gradient-to-r from-black to-gray-800 text-white text-sm px-3 py-1.5 rounded-full hover:from-gray-700 hover:to-gray-800 transition-all shadow-sm w-full sm:w-auto justify-center sm:justify-start"
            >
            ...
            <div className="flex items-center gap-4 justify-center sm:justify-end">
              ...
              {(status === 'completed' || status === 'failed') && (
                <button
                  ...
                  } text-white text-sm px-3 py-1.5 rounded-full transition-all shadow-sm w-full sm:w-auto justify-center`}
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-4">
            <h1 className="text-lg sm:text-xl font-normal bg-gradient-to-r from-[#1B1B1B] to-[#4A4A4A] bg-clip-text text-transparent">
              Browser Automation
            </h1>
            <p className="text-[#1B1B1B]/60 mt-2 text-sm">Session ID: {resolvedParams.id}</p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
            {/* Browser Stream */}
            <div className="lg:col-span-2 bg-white/70 backdrop-blur-sm rounded-[20px] sm:rounded-[28px] border border-black/5 p-3 sm:p-4 md:p-6 shadow-sm">
              <div className="relative h-[400px] sm:h-[500px] md:h-[600px] rounded-xl sm:rounded-2xl overflow-hidden border border-black/5 flex flex-col items-center justify-center bg-gray-50">
                {renderBrowserContent()}
              </div>
            </div>

            {/* Agent Progress */}
            <div className="bg-white/70 backdrop-blur-sm rounded-[20px] sm:rounded-[28px] border border-black/5 p-3 sm:p-4 md:p-6 shadow-sm">
              <h2 className="text-xl sm:text-2xl font-normal mb-4 md:mb-6 text-[#1B1B1B]">Live Progress</h2>
              <div className="relative">
                <div className="space-y-4 h-[300px] sm:h-[400px] md:h-[520px] overflow-y-auto pr-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-white [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:border [&::-webkit-scrollbar-track]:border-gray-200 [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:border [&::-webkit-scrollbar-thumb]:border-white">
                  {steps.length > 0 ? (
